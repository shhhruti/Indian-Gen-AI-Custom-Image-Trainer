...
export async function deleteModel(id: number, model_id: string, model_version: string) {
    ...
    if (model_version) {
      try {
        const res = await fetch(`https://api.replicate.com/v1/models/ADDY/${model_id}/versions/${model_version}`, {
          method: 'DELETE',
          headers: {
    ...
    if (model_id) {
        try {
          const res = await fetch(`https://api.replicate.com/v1/models/ADDY/${model_id}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${process.env.REPLICATE_API_TOKEN}`,
            },
