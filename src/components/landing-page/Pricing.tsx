  ...
  return (
    <section className='w-full bg-muted flex flex-col items-center justify-center'>
      ...
      <p className='text-base text-muted-foreground max-w-3xl'>
        Chooes an affordable plan that is packed with the best features for engaging your audience, creating customer loyalty and driving sales. 
      </p>
        </div>
        ...
        products.map(product => {
            ...
            return <div key={product.id} className={cn('border bg-background rounded-xl shadow-sm h-fit divide-y divide-border',
                product.name?.toLowerCase() === mostPopularProduct.toLowerCase() ? "border-primary bg-background drop-shadow-md scale-105"
                : "border-border"
